...
std::uint32_t act01::precalcul(const day02::data::reports &reports) const
{
    ...
    for (auto &&report : reports)
    {
        ...
        std::cout << std::endl;
        const auto isSortedIncrease = std::ranges::is_sorted(report);
...
std::uint32_t act01::calcul(const day02::data::reports &reports) const
{
    ...
    // for (auto &&report : reports)
    // {
    //     if (std::ranges::is_sorted(report) || std::ranges::is_sorted(report, std::ranges::greater{}))
    //     {
    //         auto max_difference = std::transform_reduce(
    //             d.begin() + 1,
    //             d.end(),
    //             d.begin(),
    //             0,
